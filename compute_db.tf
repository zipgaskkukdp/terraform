...
# ==========================================
# 2. APP 계층 (Private - 내부 ALB + ASG)
# ==========================================
resource "aws_lb" "app_alb" {
  name               = "lso-app-alb"
  internal           = true # 외부 접근 차단, 내부(WEB)에서만 접근
  load_balancer_type = "application"
  security_groups    = [aws_security_group.app_alb_sg.id]
  subnets            = [aws_subnet.app_pri.id, aws_subnet.app_pri_2c.id]
}
...
EOF
  ...
  tag_specifications {
    resource_type = "instance"
    tags          = { Name = "LSO-PRI-APP-ASG" }
  }
}

resource "aws_autoscaling_group" "app_asg" {
  name                = "lso-app-asg"
  vpc_zone_identifier = [aws_subnet.app_pri.id, aws_subnet.app_pri_2c.id]
  min_size            = 2
  max_size            = 4
  desired_capacity    = 2
...
# ==========================================
# 3. DB & Storage 계층 (RDS + S3)
# ==========================================
resource "aws_db_subnet_group" "rds_sub" {
  name       = "lso-rds-sg"
  subnet_ids = [aws_subnet.db_pri.id, aws_subnet.db_pri_2c.id]
}
